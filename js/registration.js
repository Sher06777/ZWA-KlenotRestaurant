...
export function initRegistration() {
  ...
  if (!registerForm) { console.warn("register-form not found"); return; }
  ...
  function clearFieldError(input) { if (!input || !input.parentElement) return; const errorEl = input.parentElement.querySelector('.error-message'); if (errorEl) { errorEl.textContent = ''; errorEl.classList.remove('active'); } }
  function clearAllErrors() { registerForm.querySelectorAll('input').forEach(input => clearFieldError(input)); }
  function validateInput(input) {
    if (!input) return true;
    const value = (input.value || '').trim();
    clearFieldError(input);
    if (input.required && !value) { showFieldError(input, `The ${input.name} field is required`); return false; }
    if (input.type === 'email' && value) { const emailReg = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; if (!emailReg.test(value)) { showFieldError(input, 'Please enter a valid email'); return false; } }
    return true;
  }

  registerForm.querySelectorAll('input').forEach(input => { input.addEventListener('blur', () => validateInput(input)); input.addEventListener('input', () => clearFieldError(input)); });

  if (!registerButton) { console.warn('register button not found'); return; }
  ...
  registerButton.addEventListener('click', async (e) => {
    ...
    clearAllErrors();
    let valid = true;
    registerForm.querySelectorAll('input').forEach(input => { if (!validateInput(input)) valid = false; });
    ...
    try {
      ...
      if (window.CSRFManager && typeof window.CSRFManager.isInitialized === 'function') {
        if (!window.CSRFManager.isInitialized()) console.warn('CSRFManager not initialized at registration time.');
      }

      if (result.success) {
        if (result.user) window.user = result.user;
        else window.user = { id: result.user_id || null, name: result.user_name || registerForm.login?.value || '', email: result.user_email || registerForm.email?.value || '' };

        if (typeof window.onLoginOrRegister === 'function') window.onLoginOrRegister(window.user);
        else if (typeof initPersonalAccount === 'function') initPersonalAccount(window.user);
      } else {
        if (Array.isArray(result.fields) && result.fields.length) {
          ...
          if (input) showFieldError(input, result.message || 'Invalid value');
